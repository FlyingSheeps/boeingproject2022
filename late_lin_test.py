...
T = [[1+0j,1+0j,1+0j,1+0j],
                [8.248+0j,-0.6322+0.07478j,-0.6322-0.07478j,-0.02019+0j],
                [2.269+0j,0.9201+2.148j,0.9201-2.148j,2.2+0j],
                [-0.3659+0j,0.01483-0.03125j,0.01483+0.03125j,1.935+0j]]
T = np.array(T)
LAMBDA = np.diag([-22.54+0j,-9.786-15.59j,-9.786+15.59j,-0.01043+0j])

B =  np.array([[31.62547*0.5],
                 [0],
                 [-495.5809*0.5],
                 [0]])
...
print(sys)
#根軌跡
roots_all, gains_all = rlocus(sysall,xlim=[-30,30],ylim=[-30,30])
print(roots_all)
print(gains_all)
plt.savefig("late/rlocus_all_late.png")
plt.xlim([-5,5])
plt.ylim([-5,5])
plt.savefig("late/rlocus_all_spiral.png")
plt.clf()

roots, gains = rlocus(sys*controltf,xlim=[-30,30],ylim=[-30,30])
plt.savefig("late/rlocus_late.png")
print(roots)
print(gains)
plt.xlim([-5,5])
plt.ylim([-5,5])
plt.savefig("late/rlocus_spiral.png")
plt.clf()

bode(sys,servo,controltf,sysall)
plt.legend()
plt.savefig("late/bode_late.png")
plt.clf()